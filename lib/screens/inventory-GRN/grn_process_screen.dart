...
class GRNProcessScreen extends StatefulWidget {
  final TransferOrder2 order;
  final int index;
  final Map<String, dynamic>? userObj;

  const GRNProcessScreen({
    super.key,
    required this.order,
    required this.index,
    this.userObj,
  ...
  @override
  State<GRNProcessScreen> createState() => _GRNProcessScreenState();
}

class _GRNProcessScreenState extends State<GRNProcessScreen> {
  ...
  void dispose() {
    ...
    varianceReasonControllers.values
  ...
  void _simulateIMEIScan(String itemKey) {
    // Simulate IMEI scanner - replace with actual scanner implementation
    showDialog(
      context: context,
      builder: (context) {
        String scannedIMEI = '';
        return AlertDialog(
          title: Text('Scan IMEI'),
          content: TextField(
            onChanged: (value) => scannedIMEI = value,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                if (scannedIMEI.isNotEmpty) {
                  setState(() {
                    scannedIMEIs[itemKey]!.add(scannedIMEI);
                  });
                }
                  ...
                  if (currentStep > 0)
                    Expanded(
                      child: ElevatedButton(
                        onPressed: isProcessingGRN ? null : _previousStep,
                        child: Text(
                          'Previous',
  ...
  String _getStep2Title() {
    ...
    bool hasNonSerializedItems =
        discrepancyItems.any((item) => !item.serialized);
                  ...
                  if (scannedIMEIs[itemKey]!.isNotEmpty)
                    Container(
                      ...
                      child: Wrap(
                        ...
                        runSpacing: 4,
                        children:
                            scannedIMEIs[itemKey]!.asMap().entries.map((entry) {
                          int index = entry.key;
                  ...
                  OutlinedButton.icon(
                    ...
                    icon: Icon(
                        item.serialized
                            ...
                            : Icons.camera_alt,
                        size: 18),
                    label: Text(item.serialized ? 'Scan IMEI' : 'Take Photo'),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildSummaryRow(
                      'Total Items', '${widget.order.items.length}'),
                  _buildSummaryRow(
                      'Status',
                      _hasDiscrepancies()
                          ? 'Partially Received'
                  ...
                  if (_hasDiscrepancies())
                    _buildSummaryRow(
                        ...
                        '${widget.order.items.where((item) {
                          String itemKey =
                              '${item.model}_${item.primaryIdentifier}';
                          int actual = actualQuantities[itemKey] ?? 0;
                          return actual != item.quantity;
                        }).length}'),
                ],
  ...
  Color _getNextButtonColor() {
    if (isProcessingGRN) return Colors.grey[400]!;
